/* 25 · Menú de mesa: chips pegajosos con scroll-spy, "+" / "− n +" por tarjeta y hoja de detalle.
...
(function () {
  ...
  function init() {
    ...
    /* ---------- Chips: salto sin smooth + scroll-spy ---------- */
    ...
    function offset() { return headerH() + (bar ? bar.offsetHeight : 0) + 6; }
    chips.forEach(function (c, i) {
      c.addEventListener('click', function (e) {
        if (!cats[i]) return;
        e.preventDefault();
        window.scrollTo(0, cats[i].getBoundingClientRect().top + window.pageYOffset - offset() + 20);
        setOn(i);
      });
    });
    var on = -1;
    function setOn(i) {
      ...
      var c = chips[i];
      if (c && strip) {
        var x = c.offsetLeft - (strip.clientWidth - c.offsetWidth) / 2;
    ...
    function spy() {
      ticking = false;
      var y = offset() + 10, idx = 0;
      for (var i = 0; i < cats.length; i++) { if (cats[i] && cats[i].getBoundingClientRect().top <= y) idx = i; }
      setOn(idx);
    }
