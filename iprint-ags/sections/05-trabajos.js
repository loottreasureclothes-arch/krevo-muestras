/* 05 Trabajos (clonado de closetdoor/sections/02-trabajos.js): slider propio, tercia/coverflow en
   celular. La tarjeta sigue al dedo 1:1, un flick avanza solo 1, el vertical deja pasar el scroll,
   resorte al soltar, parallax interno y cinta metrica. Profundidad: las tarjetas entran desde el
   fondo al llegar a la seccion (una vez), se inclinan con la velocidad del arrastre y, en compu,
   la del frente sigue al mouse. Diferencia con Closet&Door: "Cotizar" es un <button> que abre la
   hoja compartida (IP.sheet) con la foto en grande, no un link directo a WhatsApp (cuida el
   presupuesto de wa.me de la página). */
(function () {
  "use strict";
  function init() {
    ...
    // "Cotizar": abre la hoja compartida (IP.sheet) con la foto real en grande y "Cotizar uno así"
    Array.prototype.forEach.call(sec.querySelectorAll(".tj-cta"), function (b) {
      b.addEventListener("click", function (e) {
        e.stopPropagation();
        var k = b.getAttribute("data-k"), n = b.getAttribute("data-n");
        if (!window.IP || !IP.sheet) return;
        IP.sheet({
          t: n, d: b.getAttribute("data-d") + ". Trabajo real de iPrint.",
