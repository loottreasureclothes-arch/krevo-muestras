 ...
 * El botón de pago iría en la hoja, junto a "Enviar por WhatsApp" (ver 26-pedido.html, .dp-pd-actions).
 */
(function () {
  'use strict';
  var WA = (window.DP && window.DP.WA) || '524491554787'; // ver PENDIENTES.md
  var KEY = 'dp_pedido';
   ...
   * window.dpLayer.open(nombre, cerrar) / .close(nombre). Lo usan también el diálogo del platillo (25-menu.js). */
  var layer = window.dpLayer = (function () {
    var stack = [], skip = 0;
    ...
    return {
      open: function (name, fn) {
        ...
        try { history.pushState({ dpLayer: name }, ''); } catch (e) {}
      },
      close: function (name) {
        var i = -1;
  ...
  var S = { lines: [], note: '', mode: 'mesa', mesa: '', suc: 'galeana', hora: '', fecha: '', personas: '', ts: 0 };
  var SUC = { galeana: 'Galeana Sur 382', cenaduria: 'Cenaduría Paseo de las Maravillas' };
  function sucName() { return SUC[S.suc] || SUC.galeana; }
  ...
  function waText() {
    var t = 'Hola, quiero hacer un pedido en Petra (' + sucName() + ').\n';
  ...
  function render() {
    ...
    bar.hidden = n === 0;
    document.documentElement.classList.toggle('dp-pd-has', n > 0);
    $('.dp-pd-bar-n', bar).textContent = n;
    $('.dp-pd-bar-t', bar).textContent = money(tot);
    $('.dp-pd-bar-mesa', bar).textContent = S.mode === 'mesa' && S.mesa ? 'Mesa ' + S.mesa : '';
    $('.dp-pd-mini-n', mini).textContent = n;
    mini.hidden = n === 0;
    mini.setAttribute('aria-label', 'Ver mi pedido: ' + n + (n === 1 ? ' producto' : ' productos'));
    sheet.querySelectorAll('input[name="dp-pd-suc"]').forEach(function (r) { r.checked = r.value === S.suc; });
    var sb = document.getElementById('dp-mm-mesa');
    ...
    if (!S.lines.length) {
      ...
      e.className = 'dp-pd-empty';
      e.textContent = 'Tu pedido está vacío. Agrega platillos o bebidas desde el menú.';
      list.appendChild(e);
    }
    S.lines.forEach(function (l) {
      var li = document.createElement('li');
      li.className = 'dp-pd-line';
      li.innerHTML = '<div class="dp-pd-line-tx"><b></b><small></small></div>' +
        '<div class="dp-mm-step dp-pd-step"><button type="button" data-d="-1" aria-label="Quitar uno">&minus;</button><output></output><button type="button" data-d="1" aria-label="Agregar uno">+</button></div>' +
        '<span class="dp-pd-line-t"></span>';
      $('b', li).textContent = l.name;
      $('small', li).textContent = (l.opt ? l.opt + ' · ' : '') + money(l.price) + ' c/u';
      $('output', li).textContent = l.qty;
      $('.dp-pd-line-t', li).textContent = money(l.price * l.qty);
  ...
  function lock(on) { document.documentElement.classList.toggle('dp-mm-lock', on); }
  function openSheet() {
    ...
    setTimeout(function () { var c = $('.dp-pd-x', sheet); c && c.focus({ preventScroll: true }); }, 30);
  }
  ...
  function showMesero() {
    ...
    $('.dp-ms-mesa', mesero).textContent = S.mesa;
    $('.dp-ms-suc', mesero).textContent = 'Petra · ' + sucName();
    var ul = $('.dp-ms-list', mesero);
    ul.innerHTML = '';
    S.lines.forEach(function (l) {
      var li = document.createElement('li');
    ...
    var nt = $('.dp-ms-note', mesero);
    nt.hidden = !S.note.trim();
    nt.textContent = S.note.trim() ? 'Nota: ' + S.note.trim() : '';
    $('.dp-ms-total', mesero).textContent = money(total());
    $('.dp-ms-hora', mesero).textContent = hhmm();
    mesero.hidden = false;
    lock(true);
    layer.open('mesero', function () { closeMesero(true); });
    fireCheckout('mesero');
    setTimeout(function () { $('.dp-ms-x', mesero).focus({ preventScroll: true }); }, 30);
  }
  ...
  function scrollToMenu() {
    var m = document.querySelector('#menu .dp-mm-head') || document.getElementById('menu');
    if (!m) return;
  ...
  function init() {
    bar = $('#dp-pd-bar'); mini = $('#dp-pd-mini'); sheet = $('#dp-pd-sheet'); mesero = $('#dp-pd-mesero');
    if (!bar || !mini || !sheet || !mesero) return;
    list = $('.dp-pd-list', sheet); totalEl = $('.dp-pd-total b', sheet);
    waBtn = $('#dp-pd-wa'); meseroBtn = $('#dp-pd-mesero-btn'); err = $('#dp-pd-err');
    load();

    // ?suc=galeana|cenaduria fija la sucursal (el QR de cada mesa la trae); si no viene, se elige en la hoja
    var qs = ((location.search.match(/[?&]suc=([a-z]+)/i) || [])[1] || '').toLowerCase();
    if (SUC[qs]) { S.suc = qs; sucFixed = true; save(); }
    var sucF = $('.dp-pd-suc', sheet);
    if (sucF) sucF.hidden = sucFixed;
    sheet.querySelectorAll('input[name="dp-pd-suc"]').forEach(function (r) {
    ...
    if (qm) {
      ...
      var badge = document.getElementById('dp-mm-mesa');
    ...
    // Campos de la hoja
    var note = $('#dp-pd-note'), mesa = $('#dp-pd-mesa'), hora = $('#dp-pd-hora'), fecha = $('#dp-pd-fecha'), hora2 = $('#dp-pd-hora2'), pers = $('#dp-pd-personas');
    ...
    var radios = sheet.querySelectorAll('input[name="dp-pd-mode"]');
    ...
    if (menuSec && 'IntersectionObserver' in window) {
      new IntersectionObserver(function (es) {
        document.documentElement.classList.toggle('dp-pd-far', !es[0].isIntersecting);
    ...
    if (hero && 'IntersectionObserver' in window) {
      new IntersectionObserver(function (es) {
        document.documentElement.classList.toggle('dp-pd-hero', es[0].isIntersecting);
    ...
    $('.dp-ms-x', mesero).addEventListener('click', closeMesero);
    $('#dp-pd-clear').addEventListener('click', function () {
