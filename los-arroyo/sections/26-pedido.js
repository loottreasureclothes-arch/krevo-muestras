...
(function () {
  'use strict';
  var WAS = { 'santa-anita': '524495542823', chicahuales: '524492310357', poniente: '524492310357' }; // Santa Anita: el WhatsApp que contestó; Chicahuales: lona con logo de WhatsApp; López Mateos: teléfono de Google (confirmar)
  var KEY = 'la_pedido';
  ...
  function load() {
    try {
      ...
      if (!SUC[S.suc]) S.suc = 'santa-anita';
    } catch (e) {}
  }
  function save() {
    S.ts = Date.now();
  ...
  function waText() {
    var t = 'Hola, quiero hacer un pedido en Los Arroyo (' + sucName() + ').\n';
  ...
  function waUrl() { return 'https://wa.me/' + (WAS[S.suc] || WAS['santa-anita']) + '?text=' + encodeURIComponent(waText()); }
  function hasAsk() { return S.lines.some(function (l) { return !l.price; }); }
  ...
  function render() {
    ...
    S.lines.forEach(function (l) {
      ...
      $('small', li).textContent = (l.opt ? l.opt + ' · ' : '') + (l.price ? money(l.price) + ' c/u' : 'precio en sucursal');
      $('output', li).textContent = l.qty;
      $('.lm-pd-line-t', li).textContent = l.price ? money(l.price * l.qty) : '';
  ...
  function showMesero() {
    ...
    $('.lm-ms-suc', mesero).textContent = 'Los Arroyo · ' + sucName();
  ...
  function init() {
    ...
    // Campos de la hoja
    var note = $('#lm-pd-note'), mesa = $('#lm-pd-mesa'), hora = $('#lm-pd-hora'), fecha = $('#lm-pd-fecha'), hora2 = $('#lm-pd-hora2'), pers = $('#lm-pd-personas');
