...
(function () {
  ...
  function init() {
    ...
    function paint() {
      ...
      $d('.lm-mm-dlg-price').textContent = money(price());
      $d('.lm-mm-dlg-add').textContent = 'Agregar ' + qty + ' · ' + money(price() * qty);
    }
    ...
    // API: abrir una categoría desde otras secciones (cuadritos del hero, 2x1). Links <a href="#m-xxx"> fuera de los chips.
    window.lmMenuGo = function (id) {
      var i = -1;
      cats.forEach(function (c, j) { if (c && c.id === id) i = j; });
      if (i < 0) return false;
      if (q && q.value) { q.value = ''; search(); }
      setOn(i);
      toTop(true);
    ...
    document.addEventListener('click', function (e) {
      ...
      if (!a || root.contains(a) && a.classList.contains('lm-mm-chip')) return;
      if (window.lmMenuGo(a.getAttribute('href').slice(1))) { e.preventDefault(); e.stopPropagation(); }
    }, true);
    // Arranca en la primera categoría (Rollos clásicos); #m-xxx en la URL abre esa
    setOn(0);
    if (/^#m-/.test(location.hash)) setTimeout(function () { window.lmMenuGo(location.hash.slice(1)); }, 60);
  }
